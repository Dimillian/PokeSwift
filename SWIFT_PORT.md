...
- Updated `./scripts/launch_app.sh` with an interactive live-watch mode that keeps the launch script attached, tails fresh `session_events.jsonl` entries, polls `/telemetry/latest`, and quits the app cleanly on `Ctrl-C` for manual debugging sessions.
...
- Changed scheduled battle presentation beats so every message-bearing combat/result/EXP/level-up page now pauses for `A` instead of auto-advancing on timers, while preserving the existing queued-dialogue flow for trainer prompts, capture text, payouts, and other paginated battle copy.
